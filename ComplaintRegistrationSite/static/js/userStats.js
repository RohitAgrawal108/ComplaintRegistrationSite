...
const updateTopMonthsUI = (topMonth, type) => {
  if (type === "complaints") {
    document.querySelector(".complaint-top-month").textContent = getHumanMonth(
      Object.keys(topMonth)[0]
    );
    document.querySelector(
      ".complaint-top-month-value"
...
const updateThisMonthUI = (data = [], type = "complaints") => {
  ...
  if (type === "complaints") {
    document.querySelector(".complaint-this-month").textContent = getHumanMonth(
      Object.keys(currentMonthData)[0]
    );
    document.querySelector(
      ".complaint-this-month-value"
...
const formatStats = (data = {}, type = "complaints") => {
  ...
  if (type === "complaints") {
    updateThisMonthUI(s, "complaints");
  }
  if (type === "income") {
...
const fetchData = () => {
  const promise1 = fetch("/complaint_summary_rest")
    .then((res) => res.json())
  ...
  Promise.all([promise1, promise2, promise3, promise4])
    .then((data) => {
      ...
      formatStats(thisYearComplaints.this_year_complaints_data, "complaints");
